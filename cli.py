...
class CLI( Cmd ):
    ...
    def do_multiiperf( self, line ):
    """Multi iperf UDP traffic between hosts"""
        args = line.split()
        if len(args) == 1:
            udpBw = args[ 0 ]
            self.mn.multiiperf(udpBw)
        elif len(args) == 2:
            udpBw = args[ 0 ]
            period = args[ 1 ]
            err = False
            self.mn.multiiperf(udpBw, float(period))
        else:
            error('Invalid number of args: multi_iperf udpBw period\n' +'udpBw examples: 1M 120\n')



    def do_probiperf(self, line):
    """Multiiperf UDP test with probablity"""
        args = line.split()
        if len(args) == 1:
            udpBw = args[ 0 ]
            self.mn.multiiperf(udpBw)
            ...
            period = args[ 1 ]
            err = False
            self.mn.probiperf(udpBw, float(period))
        else:
            error('invalid number of args: multiiperf udpBw period\n' +'udpBw examples: 1M 120\n')
