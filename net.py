...
class Mininet( object ):
    ...
    def single_iperf( self,hosts=None, udpBw='10M', period=60, port=5001):
        ...
        client.cmd(
            ...
            +' > /home/Mininet/log/' + 'client' + filename +'&')
    
    def multiiperf(self, bw, period=60):
        base_port = 5001
        server_list = []
        ...
        for i in xrange(0, _len):
            ...
            self.single_iperf(hosts = [client, server], udpBw=bw, period= period, port=base_port)
            sleep(.10)
            base_port += 1
        sleep(period)
        print "Test Finished"
    ...
    def probpick( self, _list, probabilities):
    ...
    def probiperf(self, bw, period = 60, a = 1,b = 4,c = 8,pa = 0.6,pb = 0.3):
        base_port = 5001
        server_list = []
        client_list = []
        client_list = [h for h in self.hosts]
        ...
        for key in xrange(_len):
            ...
            server = self.probpick(access_host,p_list)
            server_list.append(server)
            self.single_iperf(hosts = [client, server], udpBw=bw, port=base_port)
            sleep(.10)
        sleep(period)
        print "Test Finished"
